...
|Class_apparent_power|功率计，单位：VA
|Class_aqi|空气质量指数(无单位)|
|Class_area|面积，单位：m², cm², km², mm², in², ft², yd², mi², ac, ha|
|Class_atmospheric_pressure|大气压力，单位：cbar, bar, hPa, mmHg, inHg, kPa, mbar, Pa 或 psi|
|Class_battery|电池电量，单位：%|
|Clase_blood_glucose_concentration|血氧浓度，单位：mg/dL,mmol/L|
|Class_carbon_dioxide|二氧化碳浓度，单位：ppm|
|Class_carbon_monoxide|一氧化碳浓度，单位：ppm|
|Class_current|电流，单位：A，mA|
|Class_data_rate|传输速率，单位：bit/s, kbit/s, Mbit/s, Gbit/s, B/s, kB/s, MB/s, GB/s, KiB/s, MiB/s 或 GiB/s|
|Class_data_size|数据大小，单位：bit, kbit, Mbit, Gbit, B, kB, MB, GB, TB, PB, EB, ZB, YB, KiB, MiB, GiB, TiB, PiB, EiB, ZiB 或 YiB|
|Class_date|日期字符串（ISO 8601）|
|Class_distance|距离，单位：km, m, cm, mm, mi, nmi, yd, 或 in|
|Class_duration|持续时间，单位：d, h, min, s, 或 ms|
|Class_energy|能耗，单位：J, kJ, MJ, GJ, mWh, Wh, kWh, MWh, GWh, TWh, cal, kcal, Mcal, 或 Gcal|
|Class_energy_storage|能量存储，单位：J, kJ, MJ, GJ, mWh, Wh, kWh, MWh, GWh, TWh, cal, kcal, Mcal, 或 Gcal|
|Class_ha_enum|一组有限的（非数字）状态|
|Class_frequency|频率，单位：Hz, kHz, MHz, 或 GHz|
|Class_gas|气体容量，单位：m³, ft³ 或 CCF|
|Class_humidity|空气湿度，单位：%|
|Class_illuminance|光照度，单位： lx|
|Class_irradiance|辐射，单位：W/m² 或 BTU/(h⋅ft²)|
|Class_moisture|水分占比，单位：%|
|Class_monetary|货币（[ISO 4217](https://en.wikipedia.org/wiki/ISO_4217#Active_codes)）|
|Class_nitrogen_dioxide|二氧化氮，单位：µg/m³|
|Class_nitrogen_monoxide|一氧化氮，单位： µg/m³|
|Class_nitrous_oxide|一氧化二氮，单位：µg/m³|
|Class_ozone|臭氧，单位： µg/m³|
|Class_ph|PH，单位：pH|
|Class_pm1|PM1，单位：µg/m³|
|Class_pm25|PM2.5，单位：µg/m³|
|Class_pm10|PM10，单位：µg/m³|
|Class_power_factor|功率因数（无单位），单位可以是无或 %|
|Class_power|功率，单位：mW, W, kW, MW, GW 或 TW|
|Class_precipitation|降水，单位：cm, in 或 mm|
|Class_precipitation_intensity|降水强度，单位：in/d, in/h, mm/d 或 mm/h|
|Class_pressure|压力，单位：Pa, kPa, hPa, bar, cbar, mbar, mmHg, inHg 或 psi|
|Class_reactive_power|无功功率，单位：var|
|Class_signal_strength|信号强度，单位：dB 或 dBm|
|Class_sound_pressure|声压，单位：dB 或 dBm|
|Class_speed|速度，单位：ft/s, in/d, in/h, in/s, km/h, kn, m/s, mph, mm/d, 或 mm/s|
|Class_sulphur_dioxide|二氧化硫，单位：µg/m³|
|Class_temperature|温度，单位：°C, °F 或 K|
|Class_timestamp|时间戳(ISO 8601)|
|Class_volatile_organic_compounds|挥发性有机化合物，单位：µg/m³|
|Class_volatile_organic_compounds_parts|挥发性有机化合物部分，单位：ppm 或 ppb|
|Class_voltage|电压，单位：V, mV, µV, kV, MV|
|Class_volume|体积，单位： L, mL, gal, fl. oz., m³, ft³, 或 CCF|
|Class_volume_flow_rate|体积流量，单位： m³/h, ft³/min, L/min, gal/min, 或 mL/s|
|Class_volume_storage|体积存储，单位：L, mL, gal, fl. oz., m³, ft³, 或 CCF|
|Class_water|水，单位：L, gal, m³, ft³, 或 CCF|
|Class_weight|重量，单位：kg, g, mg, µg, oz, lb, 或 st|
|Class_wind_speed|风速，单位：Beaufort, ft/s, km/h, kn, m/s, 或 mph|
...
|:---:|:---:|:---:|:---:|
|name|必选|char*|实体名称|
|entity_config_topic|可选|char*|实体配置主题|
|config_data|可选|char*|实体配置数据|
|object_id|可选|char*|实体对象ID|
|unique_id|必选|char*|实体唯一标识|
|availability_mode|可选|char*|实体可用性模式|
|availability_template|可选|char*|实体可用性模板|
|availability_topic|可选|char*|实体可用性主题|
|payload_available|可选|char*|实体可用性payload|
|payload_not_available|可选|char*|实体不可用性payload|
|device_class|可选|ha_Bsensor_class_t|实体设备类型|
|entity_category|可选|char*|实体类别|
|icon|可选|char*|实体图标|
|json_attributes_template|可选|char*|实体JSON属性模板|
|json_attributes_topic|可选|char*|实体JSON属性主题|
|qos|可选|int|实体QoS|
|retain|可选|bool|实体是否保留|
|state_class|可选|char*|实体状态类别|
|state_topic|可选|char*|实体状态主题|
|value_template|可选|char*|实体值模板|
|expire_after|可选|unsigned short|实体过期时间|
|force_update|可选|bool|实体是否强制更新|
|enabled_by_default|可选|bool|实体是否默认启用|
|state|可选|bool|实体状态|
|payload_off|可选|char*|实体关闭payload|
|payload_on|可选|char*|实体打开payload|
