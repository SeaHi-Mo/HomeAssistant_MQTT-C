...
static axk_err_t event_cb(axk_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_ERROR:
        ...
        if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT)
        {
            log_error_if_nonzero("reported from axk-tls\r\n", event->error_handle->axk_tls_last_axk_err);
...
// FreeRTOS 分配标志位掩码（通常为最高位）
#define blockALLOCATED_BIT_MASK (0x80000000UL)
...
int isPvPortMallocAllocated(void *ptr)
{
    ...
    // 1. 计算元数据地址（实际内存块前移一个 BlockLink_t 大小）
    BlockLink_t *block = (BlockLink_t *)((uint8_t *)ptr - sizeof(BlockLink_t));

    // 2. 检查标志位（xBlockSize 的最高位是否为 1）
    //    注意：FreeRTOS 在分配时会设置此位，释放时清除
    if ((block->xBlockSize & blockALLOCATED_BIT_MASK) != 0)
    {
        return 1; // 是 pvPortMalloc 分配的内存
    }

    return 0; // 非 pvPortMalloc 分配或已释放
}
/**
...
int homeAssistant_mqtt_port_stop(void)
{
    ...
    if (client != NULL)
    {
        ...
        if (ret < 0)
            HA_LOG_E("[%s:%d]mqtt disconnect error\r\n", __func__, __LINE__);
        ret = aiio_mqtt_client_stop(client);
        if (ret < 0)
            HA_LOG_E("[%s:%d]mqtt stop error\r\n", __func__, __LINE__);
    }
    else
    {
        HA_LOG_E("[%s:%d]param is null\r\n", __func__, __LINE__);
...
#ifdef CONFIG_Ai_M6x
    if (client != NULL && topic != NULL)
        aiio_mqtt_client_unsubscribe(client, topic);
    else
    {
        HA_LOG_E("[%s:%d]param is null\r\n", __func__, __LINE__);
    }
